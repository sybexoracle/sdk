...
async function basicQueriesExample() {
  ...
  try {
    ...
    const feeRecipient = await oracle.getFeeRecipient({}, { client: publicClient });
    ...
    const maxQuestionLength = await oracle.getMaxQuestionLength({}, { client: publicClient });
    console.log(`Max Question Length: ${maxQuestionLength} characters`);

    const minQuestionLength = await oracle.getMinQuestionLength({}, { client: publicClient });
    console.log(`Min Question Length: ${minQuestionLength} characters`);

    const maxTimeout = await oracle.getMaxTimeout({}, { client: publicClient });
    console.log(`Max Timeout: ${maxTimeout} seconds (${(Number(maxTimeout) / 86400).toFixed(1)} days)`);

    const minTimeout = await oracle.getMinTimeout({}, { client: publicClient });
    console.log(`Min Timeout: ${minTimeout} seconds (${(Number(minTimeout) / 60).toFixed(1)} minutes)`);

    const feeDenominator = await oracle.getFeeDenominator({}, { client: publicClient });
    console.log(`Fee Denominator: ${feeDenominator}`);

    const refundPeriod = await oracle.getRefundPeriod({}, { client: publicClient });
    console.log(`Refund Period: ${refundPeriod} seconds (${(Number(refundPeriod) / 86400).toFixed(1)} days)`);
    ...
    const RESOLVER_ROLE = "0x9d8eef2d77b408ce23bfc4866c9e6e5e4f2e2c5e6c3f7c6d6b696c6c5f5e5d5c";

    try {
      const hasAdminRole = await oracle.hasRole(
        { role: DEFAULT_ADMIN_ROLE as `0x${string}`, account: checkAddress as `0x${string}` },
        { client: publicClient }
      );
      console.log(`\n👑 Has Admin Role: ${hasAdminRole}`);

      const hasResolverRole = await oracle.hasRole(
        { role: RESOLVER_ROLE as `0x${string}`, account: checkAddress as `0x${string}` },
        { client: publicClient }
      );
