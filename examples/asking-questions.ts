...
import { oracle, type TransactionReceiptReturn } from "../src";
...
async function askingQuestionsExample() {
	...
	try {
		...
		const booleanQuestion =
			"Will Bitcoin price exceed $100,000 by December 31, 2024?";
		...
		const numericalQuestion =
			"What will be the price of ETH in USD on December 31, 2024?";
		...
		if (questionCount > 0) {
			...
			if (!question.isResolved) {
				const timeLeft =
					...
					(Date.now() / 1000 - Number(question.createdAt));
				if (timeLeft > 0) {
					console.log(
						`Time remaining: ${(timeLeft / 3600).toFixed(1)} hours`,
					);
				} else {
		...
		console.log(
			"- Use additionalData to store metadata for easier tracking",
		);
