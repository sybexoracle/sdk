...
class PriceFeedManager {
  ...
  async getPrice(symbol: string): Promise<number | null> {
    ...
    if (feed.price && feed.last_updated) {
      ...
      if (age < maxAge) {
        console.log(`Price from cache: $${feed.price} (${(age / 1000).toFixed(0)}s old)`);
        return feed.price;
      }
    }
  ...
  displayFeeds() {
    ...
    this.feeds.forEach((feed, symbol) => {
      ...
      console.log(`  Confidence: ${(feed.confidence_score * 100).toFixed(0)}%`);
      ...
      if (stats) {
        console.log(`  24h Change: ${stats.change > 0 ? '+' : ''}${stats.change.toFixed(2)}%`);
...
async function priceFeedsExample() {
  ...
  try {
    ...
    const priceUpdates = [
      { symbol: "BTC", price: 43250.75, source: "Binance", volume: 1234567 },
      { symbol: "ETH", price: 2245.30, source: "Uniswap", volume: 987654 },
      { symbol: "BNB", price: 312.45, source: "PancakeSwap", volume: 543210 },
