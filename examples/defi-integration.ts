...
class DeFiPriceOracle {
	...
	async monitorPrices() {
		...
		recentQuestions.forEach((q) => {
			...
			console.log(
				`- ${q.asset}: ${q.resolved ? "✅ Resolved" : "⏳ Pending"} (${age.toFixed(1)}h ago)`,
			);
		});
		...
		for (let i = 0n; i < questionCount; i++) {
			try {
				...
				if (
					...
					console.log(
						`Updating protocol with price: ${price} (${Number(price)})`,
					);
	...
	async checkLiquidations(
		positions: Array<{ address: string; collateral: string; debt: number }>,
	) {
		...
		for (const position of positions) {
			try {
				...
				if (!price) {
					console.log(
						`⏳ No price available for ${position.collateral}`,
					);
					continue;
				}
				...
				if (healthFactor < 150) {
					// Below 150% health factor
					console.log(
						`⚠️ POSITION AT RISK! Health factor: ${healthFactor.toFixed(2)}%`,
					);
				...
				console.log(
					`❌ Error checking position ${position.address}:`,
					error,
				);
			}
		}
...
async function defiIntegrationExample() {
	...
	try {
		...
		const mockProtocolAddress =
			"0x1234567890123456789012345678901234567890";

		for (const asset of assets) {
			try {
				const price = await priceOracle.updateProtocolPrice(
					asset,
					mockProtocolAddress,
				);
