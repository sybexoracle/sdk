...
class PredictionMarketPlatform {
  ...
  async createMarket(params: {
    ...
    switch (params.category) {
      ...
      case "categorical":
        if (!params.outcomes || params.outcomes.length < 2) {
          throw new Error("Categorical markets need at least 2 outcomes");
        }
    ...
    market.oracle_question_id = await this.getQuestionIdFromTx(tx);
  ...
  getMarketStats(market_id: string) {
    ...
    const totalBets = Array.from(outcomeTotals.values()).reduce((a, b) => a + b, 0);
    ...
    try {
      ...
      switch (market.category) {
        case "binary":
          ...
          console.log(`Winning outcome: ${winningOutcome === 0 ? "FALSE" : "TRUE"}`);
          break;
        case "categorical":
          // Parse categorical answer (implementation depends on your format)
          winningOutcome = 0; // Simplified
          console.log(`Winning outcome: ${market.outcomes?.[winningOutcome]}`);
          break;
        case "numerical":
   ...
   */
  async manuallyResolveMarket(market_id: string, outcome: number): Promise<void> {
  ...
  private async settlePositions(market_id: string, winningOutcome: number) {
    ...
    positions.forEach(pos => {
      if (pos.outcome === winningOutcome) {
        ...
        const payout = (pos.shares / totalWinningShares) * (positions.length * 100); // Simplified
        console.log(`${pos.user}: Won ${payout.toFixed(2)} (${pos.shares} shares)`);
      } else {
  ...
  listMarkets() {
    ...
    this.markets.forEach((market, id) => {
      ...
      if (stats) {
        ...
        console.log(`  Time left: ${(stats.timeRemaining / (1000 * 60 * 60)).toFixed(1)} hours`);

        if (Object.keys(stats.odds).length > 0) {
          console.log(`  Odds:`);
          Object.entries(stats.odds).forEach(([outcome, odds]) => {
            const outcomeLabel = market.outcomes?.[Number(outcome)] || outcome;
...
async function predictionMarketExample() {
  ...
  try {
    ...
    const numericalMarket = await platform.createMarket({
      question: "What will be the final price of ETH on December 31, 2024?",
      category: "numerical",
      duration: 720, // 30 days
    });
