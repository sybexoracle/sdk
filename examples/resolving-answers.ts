...
async function resolvingAnswersExample() {
	...
	if (!walletClient.account) {
		throw new Error("Wallet client must have an associated account");
	}
	...
	try {
		...
		if (!isResolver) {
			...
			console.log(
				"Continuing with example assuming you have permissions...\n",
			);
		}
		...
		try {
			...
			if (
				!question.isResolved &&
				question.questionType === QuestionType.CATEGORICAL
			) {
				...
				console.log(
					`✅ Categorical question resolved! TX: ${resolveTx}`,
				);
			}
		} catch {
			console.log(
				"❌ Could not resolve categorical question (may not exist)",
			);
		}
		...
		try {
			...
			console.log(
				"❌ Could not resolve numerical question (may not exist)",
			);
		}
		...
		try {
			...
			if (
				!question.isResolved &&
				question.questionType === QuestionType.RANGE_NUMERICAL
			) {
				...
				const resolveTx = await resolvers.rangeNumerical.resolve(
					{
						...
						proof: "This is a sample proof string",
					},
					{
						client: walletClient,
						waitForTransaction: true,
				...
				console.log(
					`✅ Range numerical question resolved! TX: ${resolveTx}`,
				);
			}
		} catch {
			console.log(
				"❌ Could not resolve range numerical question (may not exist)",
			);
		}
		...
		try {
			...
			if (!question.isResolved) {
				...
				const answerData =
					"0x" +
					Buffer.from("Result: Candidate A", "utf8").toString("hex");
				console.log(`\nProviding answer: ${answerData}`);

				const answerTx = await oracle.provideAnswer(
					{
						questionId: generalQuestionId,
						answerData: answerData as `0x${string}`,
					},
					{
						client: walletClient,
						waitForTransaction: true,
			...
			console.log(
				"❌ Could not provide answer (may not exist or no permissions)",
			);
		}
		...
		try {
			...
			console.log(
				"❌ Could not manage resolver permissions (need admin role)",
			);
		}
