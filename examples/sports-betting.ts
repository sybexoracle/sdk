...
class SportsBettingPlatform {
	...
	async addMatch(params: {
		...
		const timeout = BigInt(
			Math.floor((params.match_date.getTime() - Date.now()) / 1000) +
				24 * 60 * 60, // Match time + 24 hours buffer
		);
		...
		match.oracle_question_id = await this.getQuestionIdFromTx();
	...
	async placeBet(params: {
		...
		const odds = this.calculateOdds(params.bet_type);
	...
	async settleMatch(match_id: string): Promise<void> {
		...
		try {
			...
			console.log(`\n🏆 Match Result:`);
			console.log(
				`${match.teams.home} ${match.result.home_score || 0} - ${match.teams.away} ${match.result.away_score || 0}`,
			);
			...
			const tx = await resolvers.binary.resolve(
				{
					...
					proof: "This is a manual resolution by authorized resolver, and this serves as the proof. Source: Sybex Sports Data Feed.",
				},
				{
					client: this.walletClient,
					waitForTransaction: true,
	...
	private calculateOdds(betType: string): number {
	...
	private async settleBets(
		...
		const matchBets = Array.from(this.bets.values()).filter(
			(b) => b.match_id === match_id,
		);
		...
		matchBets.forEach((bet) => {
			...
			if (won) {
				...
				console.log(
					`✅ ${bet.user}: Won $${payout.toFixed(2)} on ${bet.selection}`,
				);
			} else {
				console.log(
					`❌ ${bet.user}: Lost $${bet.amount} on ${bet.selection}`,
				);
			}
		});
	}

	private async getQuestionIdFromTx(): Promise<bigint> {
		// Mock implementation
...
async function sportsBettingExample() {
	...
	try {
		...
		userBets.forEach((bet) => {
			console.log(
				`- ${bet.match_id}: $${bet.amount} on ${bet.selection} (${bet.status})`,
			);
		});
		...
		console.log(
			"- Multiple bet types (winner, correct score, total goals)",
		);
