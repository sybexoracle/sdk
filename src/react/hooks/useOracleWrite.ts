...
import { type UseAskOracleOptions, type UseAskBooleanOracleOptions, type UseAskNumericOracleOptions, type UseProvideAnswerOptions, type UseAddResolverOptions, type UseRemoveResolverOptions, type UseSetFeeOptions, type UseSetFeeRecipientOptions, type UseGrantRoleOptions, type UseRevokeRoleOptions, type UseRenounceRoleOptions, type OracleWriteOptions } from "../types";
...
export function useSetFeeRecipient({
  ...
  const setFeeRecipient = async (params: {
    ...
    return await writeContractAsync({
      ...
      args: [
        params.feeRecipient as `0x${string}`
      ],
      chainId: params.chainId,
    });
  };
